...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PLC unit input in lnReqAmount, we generate the URI with PLC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PLC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
